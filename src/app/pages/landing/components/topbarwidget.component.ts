...
@Component({
    ...
    template: `<a class="flex items-center" href="#">
            ...
            <span class="text-surface-900 dark:text-surface-0 font-medium text-2xl leading-normal mr-20">hygienicflow...</span>
        </a>
        ...
        <div class="items-center bg-surface-0 dark:bg-surface-900 grow justify-between hidden lg:flex absolute lg:static w-full left-0 top-full px-12 lg:px-0 z-20 rounded-border">
            <ul class="list-none p-0 m-0 flex lg:items-center select-none flex-col lg:flex-row cursor-pointer gap-8">
                <li>
                    <a (click)="router.navigate(['/app'], { fragment: 'home' })" pRipple class="px-0 py-4 text-surface-900 dark:text-surface-0 font-medium text-xl">
                        <span>Home</span>
                    </a>
                </li>
                <li>
                    <a (click)="router.navigate(['/public/features'], { fragment: 'features' })" pRipple class="px-0 py-4 text-surface-900 dark:text-surface-0 font-medium text-xl">
                        <span>Features</span>
                    </a>
                </li>
                <li>
                ...
                <li>
                    <a (click)="router.navigate(['/public/pricing'], { fragment: 'pricing' })" pRipple class="px-0 py-4 text-surface-900 dark:text-surface-0 font-medium text-xl">
                        <span>Pricing</span>
                    </a>
                </li>
            </ul>
...
export class TopbarWidget {
    ...
    ngOnInit(): void {
        ...
        this.AuthServ.isLoggedIn$.subscribe(status => {
            ...
            // console.log('TopbarWidget: isLoggedIn$ status:', status); // Use console.log for debugging
            // alert('status:' + status); // REMOVE ALERTS IN PRODUCTION CODE
        });
    }

    async logout(): Promise<void> { // Mark as void as it doesn't return anything meaningful
        const aRefreshToken = this.AuthServ.getRefreshToken();
        try {
            alert('logginig out............')
            await this.AuthServ.logout(aRefreshToken).toPromise(); // Convert Observable to Promise
            this.router.navigate(['/auth/login']); // Redirect to login after successful logout
        } catch (error) {
            console.error('Logout failed:', error);
            // Optionally show a message to the user that logout failed on backend
            // Even if backend fails, frontend tokens are cleared by clearAuthTokens()
            this.router.navigate(['/auth/login']); // Still redirect to login
        }
    }
}