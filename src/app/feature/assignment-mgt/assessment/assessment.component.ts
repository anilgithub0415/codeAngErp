...
@Component({
  ...
  imports: [ReactiveFormsModule,FormsModule,FormlyModule,//FormlyPrimeNGModule, FormlySelectModule,
      ...
      ToggleButtonModule,TableModule,AssessAssignmentTypeComponent
...
export class AssessmentComponent {
  ...
  cols!: Column[];
  expandedRows: { [key: string]: boolean } = {}; // Object to hold the state of expanded rows
  ...
  displayModalForAssessment:boolean=false;
  //-------------------------
  ...
  async   ngOnInit() {
          ...
          var whichForm='assignmentsForAssessment';
          this.formFields=
              ...
              tap((fields:FormlyFieldConfig[]) => {
                ...
                if (courseofferingField) {
                  courseofferingField.props = courseofferingField.props || {};
                                ...
                                this.facultyprofileId=16;// pending- static assignment
                                  this.loadAssignmentOfFaculty(this.tenantIdFromusercontext!,this.facultyprofileId);
...
loadAssignmentOfFaculty(ptenantId:string,facultyprofileId:number):void{
  
  //this.assignments$=
  this.assignmentService.getAssignmentsForFaculty(ptenantId,facultyprofileId).pipe(
  ...
  ).subscribe(data=>{this.assignments=data;  this.filteredAssignments.set(this.assignments);})
}
...
exportCSV(): void {
  ...
  onRowToggle(assignment: any) {
    if (this.expandedRows[assignment.id]) {
      delete this.expandedRows[assignment.id];
    } else {
      this.expandedRows = { ...this.expandedRows, [assignment.id]: true };
    }
  }

  // Utility function to format date
  formatLocalShortDate(dateString: string): Date {
    return new Date(dateString);
  }

  onViewDetails(attempt: any) {
    ...
    this.displayModalForAssessment=true;
  }
  ...
  //     console.log('Submitting grade for:', this.selectedAttempt());
  //    // console.log('Grade:', this.gradingForm.value.grade);
  //   //  console.log('Feedback:', this.gradingForm.value.feedback);
...
//pending- 
  formatLocalShortDate_earlier(date: string | Date): string {
  if (!date) {
    return '';
  }
