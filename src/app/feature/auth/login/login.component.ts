...
export class LoginComponent {
  ...
  constructor(
    ...
    //this method commented on 29Jul2026
    //was successflly in use for context selection logic, that is if one user with 2 roles loggs in then we were selecting one role for go ahead
//     onSubmit(): void {
...
//Above Login method works with context selection approach
onSubmit(): void {
  if (this.loginForm?.valid) {
    ...
    this.authService.login(UserName, password).subscribe({
      next: (response) => { 
        this.errorMessage = '';
        //------------------------------
        // Inside your LoginComponent -> onSubmit() -> next: (response) => { ... }
if (response.tenantId !== undefined) {
    localStorage.setItem('tenant_id', response.tenantId.toString());
    // Priming the active stream value immediately so Topbar hears it on init
    this.authService['activeTenantIdSubject'].next(Number(response.tenantId)); 
}

        //------------------------------
        // 1. Process and save tokens/permissions immediately
        const userPermissions = response.permissions || [];
        localStorage.setItem('user_permissions', JSON.stringify(userPermissions));
        ...
        // 2. Hydrate global UI streams immediately
        const userId = this.authService.getUserId();
        if (response.tenantId !== undefined && userId) {
          this.layoutService.loadUserPreferences(Number(response.tenantId), userId);
        }

        // 3. Setup reactive contexts for SuperAdmin components to track
        this.authService.updateCurrentUserRoleAndPermissions();
        this.authService.setLoginStatus(true);

        // 4. Route right into the application - No dialogue window required!
        this.router.navigate(['/app']); 
      },
      error: (error) => {
