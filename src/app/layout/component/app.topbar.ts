...
@Component({
    ...
    template: ` <div class="layout-topbar">
        ...
        <ng-container *ngIf="auth.isLoggedIn$ | async">
          ...
          <!-- <p-button label="Logout" icon="pi pi-power-off" (click)="logout()" /> -->
        </ng-container>
        <ng-container *ngIf="!(auth.isLoggedIn$ | async)">
        ...
        <div class="layout-topbar-actions">
            ...
            <div class="layout-topbar-menu hidden lg:block">
                <div class="layout-topbar-menu-content">
                    ...
                    <ng-container *ngIf="currentUser && currentUser.profilePictureUrl; else defaultUserIcon">
                    <img [src]='currentUser.profilePictureUrl' alt="User Avatar" class="profile-avatar" />
                </ng-container>
...
export class AppTopbar implements OnInit {
    ...
    items!: MenuItem[];usersID!:number|null;
 currentUser!:Partial<User>;// | null = null;
 
 availableContexts: AvailableContext[] | null = null;
 ...
 loadingContext:boolean=false;
    ...
    constructor(public layoutService: LayoutService,public auth:AuthService, private router:Router,
      ...
      private messageService: MessageService, private eventbusService:EventBusService) {}

    ngOnInit(): void {
        
        this.eventbusService.on(Events.contextSelected,()=>{
        ...
        this.items = [
            {
                label: 'Profile',
                icon: 'pi pi-user',
                routerLink: '/auth/profile' // Example: navigate to a profile page
            },
            {
                label: 'Account Settings',
                icon: 'pi pi-cog',
                routerLink: '/auth/account' // Example: navigate to account settings
            },
            {
                separator: true // Adds a horizontal line
            },
            {
                label: 'Dashboard',
                icon: 'pi pi-chart-line',
                routerLink: '/app/dashboard' // Example: navigate to a personal dashboard
                
            },
            {
                label: 'Subscription',
                icon: 'pi pi-wallet',
                routerLink: '/billing/subscription', // Example: navigate to billing/subscription
                visible:true
            },
            {
                separator: true
    ...
    selectContext(context: AvailableContext): void {
        ...
        this.auth.setActiveContext(context).subscribe({
            next: () => {
                this.loadingContext=false;
                console.log('Context switched successfully to:', context);
                this.router.navigate(['/dashboard']); // Navigate to a default route or refresh current view
            },
            error: (err) => {
    ...
    async loadUserdata(): Promise<void> { 
        ...
        this.usersID=await  this.auth.getUserId();
        var uid= this.usersID ?  parseInt(this.usersID?.toString()):0
        ...
        this.userService.getUser(uid).subscribe({
            next: (data:any) => {
              this.currentUser=data;
                ...
                this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'Userdata Loaded', life: 3000 });
            },
            error: (err:any) => {
                console.error('Error loading userdata:', err);
                this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load userdata.', life: 3000 });
            }
        });
    }
